...
case = nb(
    ...
    code("import cartopy.crs as ccrs, cartopy.feature as cfeature",
        ...
        "        # CIG derived HOUR-BY-HOUR (per-hour conditional dist), then maxed over",
         ...
         "fig.suptitle(f'FIGS vs SPC Day-1 outlooks — valid {RUN+timedelta(hours=min(FXX)):%Y-%m-%d %HZ}–{RUN+timedelta(hours=max(FXX)):%Y-%m-%d %HZ}', y=1.02); plt.tight_layout(); plt.show()"),
